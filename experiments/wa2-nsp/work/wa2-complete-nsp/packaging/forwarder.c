...
/* Host packager supplies one extra RomFS file for the self-contained bundle. */
extern const unsigned char *host_bundle;
extern size_t host_bundle_size;
...
static void nacp_defaults( NacpStruct *nacp )
{
    ...
    snprintf( nacp->display_version, sizeof(nacp->display_version), "%s", "0.3.1" );
}
                                           ...
                                           const char **step )
{
    ...
    struct file_entry exefs[2], romfs[3], cnmt_entry;
    ...
    *step = "building the program";
    ...
    romfs[2] = (struct file_entry){ "/bundle.bin", host_bundle, host_bundle_size };
    if (!buf_zero( &program, sizeof(*header) ) ||
        !nca_write_pfs0( header, 0, exefs, 2, PFS0_EXEFS_HASH_BLOCK, &program ) ||
        !nca_write_romfs( header, 1, romfs, 3, &program ))
    { rc = MAKERESULT( Module_Libnx, LibnxError_OutOfMemory ); goto done; }
    ...
    *step = "building the meta";
    ...
    cnmt_header.meta_header.content_meta_count = 0; /* No ContentMetaInfo records serialized. */
    cnmt_header.meta_header.attributes = 0;
    cnmt_header.meta_header.storage_id = 0; /* Packaged content is storage-independent. */
    ...
    if (!buf_write( &cnmt, digest, sizeof(digest) ))
    { rc = MAKERESULT( Module_Libnx, LibnxError_OutOfMemory ); goto done; }
