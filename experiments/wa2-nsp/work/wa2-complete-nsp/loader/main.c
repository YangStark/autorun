...
void NX_NORETURN loadNro(void) {
    ...
    if (g_nroSize) {
       ...
       // .data + .bss
        ...
        const romfs_file *bundle = findRomfsFile(romfs_files, romfs_header.fileTableSize, dir, "bundle.bin");
        bool changed = false;
        s64 storage_size = 0;
        if (!bundle || R_FAILED(fsStorageGetSize(&s, &storage_size)) || storage_size < 0 ||
            romfs_header.fileDataOff > (u64)storage_size ||
            bundle->dataOff > (u64)storage_size - romfs_header.fileDataOff ||
            bundle->dataSize > (u64)storage_size - romfs_header.fileDataOff - bundle->dataOff)
            diagAbortWithResult(MAKERESULT(Module_HomebrewLoader, 100));
        u64 install_started = armGetSystemTick();
        bool visible_progress = install_ui_begin();
        rc = runtime_deploy(&s, romfs_header.fileDataOff + bundle->dataOff, bundle->dataSize, &changed);
        fsStorageClose(&s);
        /* Persist a small diagnostic without mixing it into the immutable bundle. */
        FsFileSystem status_fs;
        if (R_SUCCEEDED(fsOpenSdCardFileSystem(&status_fs))) {
            const char *status_path = RUNTIME_ROOT "/install-status.txt";
            char status[256];
            int length = snprintf(status, sizeof(status), "version=0.3.1\nresult=0x%x\nmode=%s\nprogress=%s\nelapsed_ms=%llu\n",
                                  rc, R_FAILED(rc) ? "failed" : changed ? "deployed" : "cached",
                                  visible_progress ? "visible" : "unavailable",
                                  (unsigned long long)(armTicksToNs(armGetSystemTick()-install_started)/1000000));
            fsFsDeleteFile(&status_fs, status_path);
            if (R_SUCCEEDED(fsFsCreateFile(&status_fs, status_path, length, 0))) {
                FsFile status_file;
                if (R_SUCCEEDED(fsFsOpenFile(&status_fs, status_path, FsOpenMode_Write, &status_file))) {
                    fsFileWrite(&status_file, 0, status, length, FsWriteOption_Flush);
                    fsFileClose(&status_file);
                    fsFsCommit(&status_fs);
                }
            }
            fsFsClose(&status_fs);
        }
        install_ui_end(rc, changed);
        if (R_FAILED(rc)) {
            if (visible_progress) selfExit();
            diagAbortWithResult(rc);
        }
        strcpy(g_nextNroPath, RUNTIME_NRO);
        snprintf(g_nextArgv, sizeof(g_nextArgv), "%s %s", RUNTIME_NRO, RUNTIME_EXE);
...
/* UI teardown must release services, not request that this NSO exit. */
...
void __libnx_initheap(void) {
    ...
    /* Fixed UI-only arena, distinct from the heap lent to the loaded NRO. */
    static u8 ui_heap[32 * 1024 * 1024] __attribute__((aligned(0x1000)));
    fake_heap_start = (char*)ui_heap;
    fake_heap_end   = (char*)ui_heap + sizeof(ui_heap);
}
...
// The temporary progress screen uses a bounded, separate allocator arena.
